...
from Entities.Structural.Header import Header
from Parser.ParseElement import ParseElement


class ParseHeaderSection:
    ...
    def parse(self, element):
        ...
        if element.find("arranged") <= element.find("header") and element.find("arranged") != -1:
            arrange = None
            for a_property in self.prop.arrange:
                ...
                if element.find(propp) != -1:
                    arrange = propp
        if arrange is None:
            raise Exception("Header section states to have arrange value but it does not have a valid value")

        color_part = element[:element.find("header")]
        ...
        if index == -1 and len(element[element.find("section")+len("section")::]) > 2 and element[element.find("section")+len("section")::].find("a"):
            raise Exception("Header section missing having clause even if list of elements is specified")
        if index != -1:
            ...
            el = Header(color, arrange, len(list_elements), list_elements)
            return el.to_json()
...
if __name__ == "__main__":
    parser = ParseHeaderSection()
    # val = parser.parse("Build a vertically arranged red header section having a download button, 5 images ")
    # print(val)
    val = parser.parse("a header section having an image")
    print(val)