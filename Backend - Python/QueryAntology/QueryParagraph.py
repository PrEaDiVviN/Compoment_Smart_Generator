...
class QueryParagraph:
    ...
    def execute(self, keys, text_properties):
        response = {}
        if len(keys) != 0 :
            response = self.genericQuery.execute(keys, self.keyToQuery, "paragraph")
        ...
        if "text_properties" in keys:
            ...
            for i in query_result["results"]["bindings"]:
                if i["individual"]["value"].lower().find("paragraph") != -1:
                    index = t
                t = t + 1
