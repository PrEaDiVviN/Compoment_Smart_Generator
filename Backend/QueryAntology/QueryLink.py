...
class QueryLink:
    ...
    def __init__(self):
        ...
        self.keyToQuery = {
                            "reference": ["?individual ins:hasReference ?reference .", "?reference"],
                            "background_color": ["?individual ins:backgroundColor ?background_color .", "?background_color"],
                            "text": ["?individual ins:containsLinkLabel ?label . ?label ins:hasLabelString ?text", "?text"]
                           }
    ...
    def execute(self, keys, text_properties):
        ...
        if "text_properties" in keys:
            ...
            for i in query_result["results"]["bindings"]:
                if i["individual"]["value"].lower().find("link") != -1:
                    index = t
                t = t + 1
